    ...
    try {
      ...
      if (post.postContent.length < 800) {
        ...
        toast({
          ...
          description: "Post content is less than 800 characters. Using original content as summary.",
        })
      } else {
  ...
  const handleDownload = async () => {
    ...
    if (summaryRef.current) {
      try {
        ...
        const blob = await new Promise((resolve) => canvas.toBlob((blob) => resolve(blob), "image/png", 1.0))
  ...
  const SummaryTemplate = () => (
    ...
    >
      <div className="space-y-6">
        {/* Header */}
        <div className={`border-b pb-6 ${isDarkTheme ? "border-gray-700 text-gray-200" : "border-gray-200"}`}>
          <h1 className="font-mono text-5xl font-bold mb-4 sm:text-[2.5rem] ssm:text-[2rem]">{post?.postTitle}</h1>
        ...
        {/* Summary Content */}
        <div className={`prose prose-lg max-w-none ${isDarkTheme ? "text-gray-200" : "text-black"}`}>{summary}</div>
    ...
    <section className="ml-[3rem] mt-[9rem] max-md:ml-[0rem]">
      <article className={`${styles.post} ${styles.postdetails}`}>
        ...
        <div className={styles.user__profile}>
          <Link href={`/profile/${post?.authorId || ""}`} className={styles.user__image}>
            <Avatar className="w-12 h-12">
              <AvatarImage src={post.authorImageUrl || "/placeholder-avatar.jpg"} alt={post.author} />
              <AvatarFallback>{post.author?.charAt(0)}</AvatarFallback>
            </Avatar>
          </Link>
          <div className={styles.user__info}>
            <div className="flex items-center justify-between">
              <Link href={`/profile/${post?.authorId || ""}`} className={styles.username}>
                {post.author}
              </Link>
              <div className="flex items-center text-sm text-gray-500 ml-10 max-sm:ml-6">
                <CalendarIcon className="mr-1 h-6 w-6" />
                <time dateTime={new Date(post._creationTime).toISOString()}>{formatDate(post._creationTime)}</time>
              </div>
            </div>

            <Badge className="mb-4 h-8 min-w-[7rem] max-w-[11rem]">{post.postCategory}</Badge>
          </div>
        </div>
        ...
        <div className={styles.postimage}>
          {post?.imageUrl && <Image src={post.imageUrl} alt="thumbnail" width={230} height={46} />}
        </div>
        <div
        ...
        <div className="text-center mb-4">
          {!isSummaryReady ? (
            <Button variant="secondary" onClick={handleSummarize} disabled={isGenerating} className="w-64 mb-4">
              {isGenerating ? "Generating Summary..." : "Generate Summary with AI"}
            </Button>
          ) : (
            <div className="flex flex-col items-center">
              <div className="flex gap-4 mb-4 max-md:flex-col">
                <Button variant="secondary" onClick={handleDownload} className="w-64">
                  Download Summary as Image
                </Button>
                <Button variant="secondary" onClick={handleThemeToggle} className="w-64">
          ...
          <div className={styles.right}>
            {user ? (
              <>
                ...
                />
                <div onClick={handleToggleCommentBox} className={`${styles.icon} mt-[-4px]`}>
                  <Comment />
              ...
              <p className="text-gray-500">Sign in to interact with this post.</p>
            )}
          </div>
        </div>
      ...
      <section className="mt-8 flex flex-col gap-5 ml-[-1rem]">
        ...
        {similarPosts && similarPosts.length > 0 ? (
          ...
          <EmptyStates title="No Similar Posts Found" buttonLink="/dashboard" buttonText="Discover more posts" />
        )}
      </section>
    </section>
  )
