...
const GeneratePost = (props: GeneratePostProps) => {
  const { isGenerating, generatePost, prompt, setPrompt } = useGeneratePost(props);
  ...
  return (
    <div>
      <div className="flex flex-col gap-2.5">
        <Label className="text-16 font-bold text-white-1">Write Your Posts</Label>
        <input
          type="text"
          value={prompt}
          onChange={(e) => setPrompt(e.target.value)}
          placeholder="Enter your prompt to Generate article with AI here"
          className="input-class font-light focus-visible:ring-offset-orange-1 h-[3rem]"
        />
        <MarkdownEditor
          value={markdown}
          height="200px"
      ...
      <p className="text-green-200 text-[.72rem] font-bold max-w-[35rem]">
        Click button only to generate Article with AI after you have input your research prompt in the PROMPT field above...
      </p>
    </div>
  );
};
